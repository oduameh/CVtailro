...
def serve_download(job_id: str, filename: str):
    ...
    if output_dir:
        if not _can_access_job(job_user_id, user_id, is_authed):
            track(
                ...
                category="download",
                job_id=job_id,
                metadata={"reason": "access_denied", "source": "local"},
            )
        ...
        if file_path.exists():
            track(
                ...
                user_id=user_id,
                job_id=job_id,
                metadata={"source": "local", "ext": Path(safe_name).suffix},
            )
            return send_from_directory(output_dir, safe_name, as_attachment=True)

    # 2. R2 cloud storage (skip "virtual" manifest rows that were never rendered)
        ...
        if job_file and job_file.r2_key:
            ...
            try:
                ...
                track(
                    ...
                    user_id=user_id,
                    job_id=job_id,
                    metadata={"source": "r2", "ext": Path(safe_name).suffix},
                )
                return redirect(url)
    ...
    track(
        ...
        user_id=user_id,
        job_id=job_id,
        metadata={"source": "db_regen", "ext": Path(safe_name).suffix},
    )
...
def _serve_from_db(db_job: TailoringJob, safe_name: str):
    ...
    if safe_name.endswith(".pdf"):
        if "cover" in safe_name.lower():
            ...
            return _regenerate_pdf(source_md, safe_name, db_job.template or "modern", job_id=db_job.id)
        ...
        if is_resume_pdf and source_md:
            ...
            for tpl in all_templates:
                if tpl in lower_safe:
                    template = tpl
                    break
            return _regenerate_pdf(source_md, safe_name, template, job_id=db_job.id)
    ...
    if safe_name.endswith(".docx"):
        if "cover" in safe_name.lower():
            ...
            source_md = _pick_source_md(db_job, safe_name)
        if source_md:
            return _regenerate_docx(
                source_md, safe_name, default_template=db_job.template or "modern", job_id=db_job.id
            )
        return jsonify({"error": "Resume content not saved. Try re-running the pipeline."}), 404
...
def _cache_regenerated_file(job_id: str | None, filename: str, data: bytes) -> None:
    """Best-effort write-back: upload regenerated bytes to R2 and record the JobFile row.
    ...
    if not job_id or not r2_storage.is_configured:
        return
    try:
        r2_key = r2_storage.upload_file(job_id, filename, file_data=data)
        job_file = JobFile.query.filter_by(job_id=job_id, filename=filename).first()
        if job_file is None:
            job_file = JobFile(job_id=job_id, filename=filename, r2_key=r2_key)
            db.session.add(job_file)
        job_file.r2_key = r2_key
        job_file.size_bytes = len(data)
        db.session.commit()
        logger.info(f"Cached regenerated file to R2: {filename}")
    except Exception as e:
        logger.warning(f"Write-back caching failed for {filename}: {e}")
        try:
            db.session.rollback()
        except Exception:
            pass
...
def _regenerate_pdf(source_md: str, filename: str, template: str, job_id: str | None = None):
    tmp_path = None
    try:
        ...
        with open(tmp_path, "rb") as f:
            pdf_data = f.read()
        _cache_regenerated_file(job_id, filename, pdf_data)
        return Response(
            pdf_data,
...
def _regenerate_docx(
    source_md: str, filename: str, default_template: str = "modern", job_id: str | None = None
):
    tmp_path = None
    try:
        ...
        with open(tmp_path, "rb") as f:
            docx_data = f.read()
        _cache_regenerated_file(job_id, filename, docx_data)
        return Response(
            docx_data,
